...
// Firebase se conecta al subir a Vercel
...
function EquipoDetail({equipo,materiales,setMateriales,specialty,onBack,user,logAction}) {
  ...
  const vincular = (mat) => {
    setMateriales(prev=>prev.map(m=>
      m.id===mat.id ? {...m, equipoIds:[...(m.equipoIds||[]), equipo.id]} : m
    ));
...
function EquiposTab({equipos,setEquipos,materiales,setMateriales,specialty,user,logAction}) {
  ...
  const save=()=>{
    ...
    const newEq = {id:Date.now(),tag:form.tag.trim(),subtag:form.subtag.trim(),nombre:form.nombre.trim(),specialty:specialty.id,lastIntervention:"—",photo:form.photo};
...
function CatalogoTab({materiales,setMateriales,specialty,user,logAction}) {
  ...
  const save=()=>{
    ...
    const newMat = {id:Date.now(),nombre:form.nombre.trim(),codigo:form.codigo.trim(),tipo:form.tipo,specialty:specialty.id,stock:form.stock||"—",icon:icons[form.tipo]||"📦",photo:form.photo,equipoIds:[]};
...
function MainApp({user,specialty,onLogout}) {
  ...
  // Firebase sync — se activa al subir a Vercel
